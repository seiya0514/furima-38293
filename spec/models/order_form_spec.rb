  ...
  describe '配送先情報の保存' do
    context '配送先情報の保存ができるとき' do
      ...
      it '電話番号が11番桁以内かつハイフンなしであれば保存できる' do
        @order_form.phone_number = 12345678900
        expect(@order_form).to be_valid
      end
    end
    ...
    context '配送先情報の保存ができないとき' do
      ...
      it '郵便番号にハイフンがないと保存できないこと' do
        ...
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include('Postcode Postal code is invalid. Enter it as follows (e.g. 123-4567)')
      end
      it '郵便番号に半角数字・ハイフン以外が含まれていると保存できない' do
        @order_form.postcode = "aaa-1111"
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include('Postcode Postal code is invalid. Enter it as follows (e.g. 123-4567)')
      end
      ...
      it '電話番号に半角数字以外が含まれていると保存できない' do
        @order_form.phone_number = "a1234567890"
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include('Phone number is invalid')
      end
